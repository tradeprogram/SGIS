...
> ⚠️ **잠정치.** 학습 데이터셋을 재구축 중이며(지오코딩 복구분 반영), 확정 후 갱신한다.

### 신규발화 GRU — 연도별 leave-one-year-out 5-fold

| horizon | AUROC | AUPRC | 기저율 | **기저율 대비 리프트** |
|---|---|---|---|---|
| t+1h | 0.804 | 0.139 | 3.03% | **4.59배** |
| t+2h | 0.800 | 0.123 | 3.03% | 4.06배 |
| t+3h | 0.784 | 0.109 | 3.03% | 3.60배 |

AUPRC 절대값이 낮아 보이지만 **기저율 대비 리프트는 기존 지속예측 모델(4.38배)과 같은 수준**이다.
차이의 대부분은 모델 성능이 아니라 문제의 희소성(9.08% → 3.03%)에서 온다.
...
4. **소형 화재 식별력이 낮다.** 0.5ha 미만 화재의 위험백분위 중앙값이 36%로 무작위에 가깝다.
5. **확률 보정 미완.** 재표본화 때문에 sigmoid 출력을 발생확률로 쓸 수 없다. 운영 단계에서 별도 calibration이 필요하다.
6. **연도별 편차.** 2022년 fold의 AUROC가 0.69로 다른 해(0.77~0.87)와 차이가 크다.
